...
### `homogeneousToPose6d(homogeneous)`
...
### `pose3dMultiply(pose_a_in_b, pose_target_in_a)`

**作用**：位姿链乘，与矩阵一致。

```math
T_{\mathrm{out}} = T(a)\,T(b),\quad \text{pose3dMultiply}(a,b)=\text{homogeneousToPose6d}\bigl(T_{\mathrm{out}}\bigr)
```
...
### `pose3dForceZDown(pose6d)`

**作用**：只重构输入位姿的姿态，使输出 Z 轴朝下；平移保持不变。

---

### `pose3dInverse(pose_a_in_b)`
...
```math
T_{\mathrm{rel}} = T(a)^{-1}\,T(b),\quad \text{pose3dOffset}(a,b)=\text{homogeneousToPose6d}(T_{\mathrm{rel}})
```
...
### `rpyDegToRotationMatrix(rx_deg, ry_deg, rz_deg, rpy_type, reference_type)`
